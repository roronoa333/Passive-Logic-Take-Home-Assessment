...
def simulate(solarHeatGain,simulationTime):
    ...
    ''' Takes in Heat generated by the solar panels, and the total simulation time, runs the simulation
        for the user defined time, reporting heat lost / generated by the various components and temperature
        gained by the fluid with in the setup at every time step within the simulation time and plots the graph to depict
        temperature gained by the fluid used over the period of simulation time versus each time step '''
